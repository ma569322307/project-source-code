...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        [self setBackgroundImage:[UIImage imageNamed:@"actionBar_background"] forToolbarPosition:0 barMetrics:0];
        ...
        self.tintColor = COLOR_RGB_HEX(0x32c3c8);
        self.barTintColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"actionBar_background"]];
...
- (void)show:(BOOL)animated completion:(void (^)(BOOL finished))completion {
    ...
    [UIView animateWithDuration:0.4 animations:^{
        self.frame = CGRectMake(0, self.superview.bounds.size.height - 388, self.superview.bounds.size.width, 44);
...
- (void)show:(BOOL)animated withOrginY:(CGFloat)OrginY completion:(void (^)(BOOL finished))completion {
    self.showHandler = completion;
    ...
    [UIView animateWithDuration:0.4 animations:^{
        self.frame = CGRectMake(0, OrginY, self.superview.bounds.size.width, 44);
    } completion:^(BOOL finished) {
        if (self.showHandler!=nil) {
            self.showHandler(YES);
        }
