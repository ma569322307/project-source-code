...
@interface XTHotEventsViewController () <XTInputViewDelegate>
...
@property (nonatomic, strong) NSURL                       *hotEventFirstImageUrl;
@property (nonatomic, copy  ) NSString                    *hotEventDesc;
...
@implementation XTHotEventsViewController

static NSString *hotEventUrl    = @"/operate/show/hotevent.json";
...
- (void)viewDidLoad {
    ...
    if([self.hotEventsTitle length] > 0) {
        self.title = self.hotEventsTitle;
    }else {
        self.title = @"热点事件";
    }
...
- (void)shareClick:(id)sender {
    ...
    if(self.firstImage) {
        [[XTShareManager sharedManager] shareWithTitle:self.hotEventsTitle withMvDesc:self.hotEventDesc withImage:self.firstImage withShareModeType:XTShareModeTypeHotEvent withPid:self.hotEventsId  withShareSheetType:XTShareSheetItemNone withCompletionBlock:nil];
    }else {
...
-(void)postBtnActionWith:(NSMutableDictionary *)info{
    ...
    [manager POST:addCommentUrl parameters:requestDic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [YYTHUD hideLoadingFrom:[UIApplication sharedApplication].keyWindow];
        [YYTHUD showPromptNoLockAddedTo:self.view withText:[error xtErrorMessage] withCompletionBlock:nil];
    }];
...
- (void)requestHotEventsInfo {
    
    [YYTHUD showLoadingAddedTo:self.view];
    
    NSDictionary *requestDic = @{@"id" : @(self.hotEventsId)};
    
    XTHTTPRequestOperationManager *manager = [XTHTTPRequestOperationManager sharedManager];
    [manager GET:hotEventUrl parameters:requestDic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        self.hotEventDesc = artistInfo.basic.content;
        artistInfo.pictures = nil;
        self.headerView = [XTHoverSwitchHeaderView hoverHeaderViewWithRecInfo:artistInfo.basic andHeaderStyle:XTHoverSwitchHeaderViewStyleHotEvent];
        [self.view insertSubview:self.headerView belowSubview:self.commentInputView];
        
        self.hotEventFirstImageUrl = artistInfo.basic.bigCover;
        ...
        self.switchCollectionView.belongId = [NSString stringWithFormat:@"%@",@(self.hotEventsId)];
        self.switchCollectionView.artistInfo = artistInfo;
        
        [self requestPicInfo];
        [self requestCommentInfo];
        
        [self.view insertSubview:self.switchCollectionView belowSubview:self.headerView];
        [self.view bringSubviewToFront:self.commentInputView];
...
- (void)downloadFirstImage {
    UIImage *headerImage = [[SDImageCache sharedImageCache] imageFromMemoryCacheForKey:self.hotEventFirstImageUrl.absoluteString];
    if(!headerImage) {
        [[SDWebImageManager sharedManager] downloadImageWithURL:self.hotEventFirstImageUrl options:SDWebImageRetryFailed | SDWebImageLowPriority
