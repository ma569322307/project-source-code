...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self imageView];
        self.checkButton.xtright = self.imageView.xtwidth - 3;
        self.checkButton.xtbottom = self.imageView.xtbottom - 3;
    }
...
- (UIButton *)photoButton{
    if (!_photoButton) {
        ...
        img = [img scaleImageWithWidth:img.size.width * 0.8];
        imgH = [imgH scaleImageWithWidth:imgH.size.width * 0.8];
...
- (void)photo
{
    ...
    if(authStatus == ALAuthorizationStatusRestricted || authStatus == ALAuthorizationStatusDenied){
        [YYTAlertView showHalfTypeAlertViewWithTitle:@"温馨提示" message:@"请在iphone的“设置-隐私-相机”选项中，允许舔访问你的相机" completaionBlock:^(NSInteger index) {
        }];
        return;
    }
